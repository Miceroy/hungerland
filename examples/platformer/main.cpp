...
namespace world {
	auto TileMap(const tmx::Map& map, const tmx::TileLayer& layer, const std::vector< std::shared_ptr<mikroplot::Texture> >& tileTextures) {
		return hungerland::MapLayer(map, layer, tileTextures);
	}
...
namespace app {
	...
	/// \param time
	///
	void render(mikroplot::Window& window, const platformer::world::World<Entity>& state, float time) {
...
int main() {
	...
	app::Functor f;
	typedef world::World<app::Entity> World;
	f.loadTexture = [&window](const std::string& fileName) {
		return window.loadTexture(fileName);
	};
	auto state = world::reset<app::Functor,app::Entity>(f, GAME_LONG_NAME, "world1");
	return 	window.run([&](mikroplot::Window& window, float dt, float time) {
		app::render(window, world::update(window, state, f, dt), time);
	});
}
